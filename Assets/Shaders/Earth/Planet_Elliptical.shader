...
Shader "Shader Forge/Planet_Elliptical" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float2 Function_node_7272( float x , float y ){
            float u = sqrt(2+pow(x,2)-pow(y,2)+2*sqrt(2)*x)/2 - sqrt(2+pow(x,2)-pow(y,2)-2*sqrt(2)*x)/2;
            float v = sqrt(2-pow(x,2)+pow(y,2)+2*sqrt(2)*y)/2 - sqrt(2-pow(x,2)+pow(y,2)-2*sqrt(2)*y)/2;
...
////// Emissive:
                float node_8488 = 0.0;
                float node_774 = length((_node_940 + ( (i.uv0 - node_8488) * (_node_6087 - _node_940) ) / (1.0 - node_8488)));
                float4 node_1071 = _Time + _TimeEditor;
                float2 node_6184 = (i.uv0*2.0+-1.0);
                float2 node_4883 = node_6184.rg;
                float2 node_7272 = Function_node_7272( node_4883.r , node_4883.g );
                float2 node_7495 = (node_7272+(node_1071.g*_Speed)*float2(1,0));
                float4 _node_4617_var = tex2D(_node_4617,TRANSFORM_TEX(node_7495, _node_4617));
                float2 node_7834 = (node_7272+(node_1071.g*_CloudsSpeed)*float2(1,0));
                float4 _Clouds_var = tex2D(_Clouds,TRANSFORM_TEX(node_7834, _Clouds));
                float node_808 = length(node_6184);
                float3 node_8599 = (_node_4617_var.rgb+(_Clouds_var.rgb*_CloudsOpacity)+(saturate(pow(node_808,_Rim))*_AtmosphereColor.rgb));
                float3 emissive = (pow((1.0 - node_774),_node_8650)*node_8599);
